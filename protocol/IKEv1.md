...
ISAKMP提供了一个认证和密钥交换的框架，但没有具体的定义它。ISAKMP被设计用来独立的密钥交换；也就是说此框架被设计成支持多种不同的密钥交换。

Oakley描述了一系列的密钥交换，被称为“模式”，详细描述了每一个提供的服务（例如，密钥的完全的后继保密(perfect forward secrecy)，身份保护和认证）。

SKEME描述了一种通用的密钥交换技术，此密钥交换技术提供了匿名性，可否认性和密钥的快速更新。
